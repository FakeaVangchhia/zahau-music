...
export const Route = createFileRoute("/courses/$slug")({
  ...
  notFoundComponent: () => (
    <div className="p-16 text-center">
      ...
      <Link to="/courses" className="mt-6 inline-block underline">Back to all courses</Link>
    </div>
  ),
});
...
function CoursePage() {
  ...
  const c = course!;
  const curriculum = (Array.isArray(c.curriculum) ? c.curriculum : []) as { term: string; topics: string[] }[];

  return (
    <>
      <section className="bg-navy text-navy-foreground py-24 px-6">
        <div className="max-w-7xl mx-auto">
          <Link to="/courses" className="font-mono text-[10px] uppercase tracking-widest text-azure hover:text-white">← All courses</Link>
          <h1 className="mt-6 font-display text-6xl md:text-8xl uppercase leading-none">{c.name}</h1>
          <p className="mt-6 max-w-2xl text-lg text-white/70">{c.tagline}</p>
          <div className="mt-10 flex flex-wrap gap-3">
            {(c.levels ?? []).map((l: string) => (
              <span key={l} className="text-xs font-mono uppercase tracking-widest border border-white/30 px-3 py-2">{l}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 max-w-7xl mx-auto grid md:grid-cols-[2fr_1fr] gap-16">
        <div>
          ...
          <div className="mt-8 space-y-6">
            {curriculum.length === 0 && <p className="text-muted-foreground">Curriculum details available on request.</p>}
            {curriculum.map((term, i) => (
              <div key={i} className="border-l-2 border-azure pl-6">
                ...
                <ul className="mt-3 space-y-1.5">
                  {term.topics.map((t) => <li key={t} className="text-sm text-muted-foreground">— {t}</li>)}
                </ul>
              </div>
            ))}
          </div>
            ...
            <>
              ...
              <ul className="mt-6 grid sm:grid-cols-2 gap-3">
                {c.outcomes.map((o: string) => (
                  <li key={o} className="border border-border bg-card px-4 py-3 text-sm">✓ {o}</li>
                ))}
              </ul>
            </>
          )}
        ...
        <aside className="space-y-8">
          ...
          {c.certification && (
            <div className="bg-secondary p-6">
              <p className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Certification</p>
          ...
          <div className="bg-navy text-navy-foreground p-6">
            ...
            <p className="mt-2 text-sm text-white/70">Submit a quick interest form and we'll schedule a free trial class within 48 hours.</p>
            <Link to="/contact" className="mt-6 inline-block bg-azure text-azure-foreground px-6 py-3 text-xs font-bold uppercase tracking-widest">Book trial class</Link>
          </div>
        </aside>
      </section>
      ...
      <section className="bg-secondary py-20 px-6">
        <div className="max-w-3xl mx-auto">
          ...
          <p className="mt-3 text-muted-foreground">Tell us your goals and we'll reply within one business day.</p>

...
export const Route = createFileRoute("/gallery")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Photos and videos from concerts, recitals, studio sessions and our Delhi campus." },
    ...
    <>
      <section className="bg-navy text-navy-foreground py-24 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <h1 className="mt-4 font-display text-6xl md:text-8xl uppercase leading-none">Moments<br />on stage.</h1>
        </div>
      </section>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
          {ITEMS.map((i, idx) => (
            <figure key={idx} className="group relative overflow-hidden bg-secondary aspect-[4/3]">
              <img src={i.src} alt={i.label} loading="lazy" className="size-full object-cover transition-transform duration-500 group-hover:scale-105" />
              <figcaption className="absolute inset-0 bg-navy/70 text-navy-foreground opacity-0 group-hover:opacity-100 transition-opacity p-6 flex flex-col justify-end">
                <span className="font-mono text-[10px] uppercase tracking-widest text-azure">{i.cat}</span>

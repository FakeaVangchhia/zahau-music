...
const CERTS = [
  { t: "ABRSM Theory Grade 3", d: "Issued Nov 2025" },
];
    ...
    <section className="bg-secondary min-h-[80vh] py-12 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-end mb-10 flex-wrap gap-4">
          <div>
            <p className="font-mono text-[10px] uppercase tracking-widest text-azure">Student portal</p>
            <h1 className="mt-2 font-display text-5xl uppercase">Welcome back.</h1>
            <p className="text-sm text-muted-foreground mt-2">{email}</p>
          </div>
          <button onClick={signOut} className="flex items-center gap-2 border border-border px-4 py-2 text-[11px] font-bold uppercase tracking-widest hover:bg-background">
                  ...
                  <div key={c.name} className="py-5">
                    <div className="flex justify-between items-end">
                      <div>
                        ...
                        <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{c.level}</div>
                      </div>
                    ...
                    <div className="mt-1 font-mono text-[10px] text-muted-foreground">{c.progress}% complete</div>
                  </div>
                ))}
              </div>
            </Card>
              ...
              <ul className="divide-y divide-border">
                {ASSIGNMENTS.map((a, i) => (
                  <li key={i} className="py-4 flex justify-between items-start">
                    <div>
                      <div className="font-mono text-[10px] uppercase tracking-widest text-azure">{a.c}</div>
              ...
              <ul className="divide-y divide-border">
                {EVENTS.map((e) => (
                  <li key={e.t} className="py-3">
                    ...
                    <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{e.w}</div>
                  </li>
                ))}
              </ul>
            </Card>
                ...
                <ul className="divide-y divide-border">
                  {CERTS.map((c) => (
                    <li key={c.t} className="py-3">
                      ...
                      <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">{c.d}</div>
                    </li>
                  ))}
                </ul>
              }

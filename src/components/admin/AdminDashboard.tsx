...
export function AdminDashboard({ email, signOut }: { email: string; signOut: () => void }) {
  const [activeTab, setActiveTab] = useState<"overview" | "appointments" | "courses" | "lessons" | "fees" | "leads" | "subscribers" | "posts">("overview");
  ...
  const navTabs = [
    ...
    { id: "appointments", label: "Appointments", icon: <CalendarCheck className="size-4" />, badge: appointments.length },
          ...
          <div className="max-w-7xl mx-auto space-y-8 animate-fadeIn">
              ...
              {activeTab === "overview" && (
                <div className="space-y-8">
                  <div>
                    <h2 className="font-display text-3xl uppercase text-foreground">System Metrics</h2>
                    <p className="text-muted-foreground text-sm mt-1">Real-time status of school data and submissions.</p>
                  </div>
                  
                  {/* Stats Grid */}
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 sm:gap-6">
                    {[
                      { id: "appointments", label: "Demo Bookings", val: appointments.length, color: "from-rose-50 to-rose-100/30 border-rose-200/80", icon: <CalendarCheck className="text-rose-600" />, textClr: "text-rose-900" },
                      { id: "courses", label: "Active Courses", val: courses.length, color: "from-blue-50 to-blue-100/30 border-blue-200/80", icon: <BookOpen className="text-blue-600" />, textClr: "text-blue-900" },
                      { id: "leads", label: "Student Leads", val: leads.length, color: "from-emerald-50 to-emerald-100/30 border-emerald-200/80", icon: <User className="text-emerald-600" />, textClr: "text-emerald-900" },
                      { id: "subscribers", label: "Newsletter Subscribers", val: subscribers.length, color: "from-purple-50 to-purple-100/30 border-purple-200/80", icon: <Mail className="text-purple-600" />, textClr: "text-purple-900" },
                      { id: "lessons", label: "Course Videos", val: lessons.length, color: "from-amber-50 to-amber-100/30 border-amber-200/80", icon: <Video className="text-amber-600" />, textClr: "text-amber-900" },
                      { id: "fees", label: "Tuition Packages", val: fees.length, color: "from-indigo-50 to-indigo-100/30 border-indigo-200/80", icon: <Calculator className="text-indigo-600" />, textClr: "text-indigo-900" },
                    ].map((s, idx) => (
                      <button 
                        key={idx} 
                        onClick={() => { setActiveTab(s.id as any); setSearchTerm(""); }}
                      ...
                      >
                        <div className="flex justify-between items-start w-full">
                          <span className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground group-hover:text-azure transition-colors">{s.label}</span>
                          {s.icon}
                        </div>
                        <span className={`font-display text-4xl ${s.textClr} group-hover:text-azure transition-colors`}>{s.val}</span>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-card border border-border/60 rounded-xl p-6">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="font-display text-lg uppercase text-foreground">Recent Appointments</h3>
                        <button onClick={() => setActiveTab("appointments")} className="text-xs text-azure hover:underline">View all</button>
                      </div>
                      {appointments.length === 0 ? (
                        <p className="text-sm text-muted-foreground/60 py-4 font-mono">No demo bookings yet.</p>
                      ) : (
                          ...
                          {appointments.slice(0, 5).map(appt => {
                            ...
                            return (
                              <div key={appt.id} className="py-3 text-xs">
                                ...
                                {parsed.day && <p className="text-muted-foreground font-mono mt-0.5">📅 {parsed.day}{parsed.time ? ` · ${parsed.time}` : ""}</p>}
                                <p className="text-muted-foreground/60 font-mono mt-0.5">{new Date(appt.created_at).toLocaleDateString()}</p>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <div className="bg-card border border-border/60 rounded-xl p-6">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="font-display text-lg uppercase text-foreground">Recent Leads</h3>
                        <button onClick={() => setActiveTab("leads")} className="text-xs text-azure hover:underline">View all</button>
                      </div>
                      {leads.length === 0 ? (
                        <p className="text-sm text-muted-foreground/60 py-4 font-mono">No incoming inquiries.</p>
                      ) : (
                        <div className="divide-y divide-border/40">
                          {leads.slice(0, 5).map(lead => (
                            <div key={lead.id} className="py-3 flex justify-between items-center text-xs">
                              <div>
                                ...
                                <p className="text-muted-foreground font-mono mt-0.5">{lead.course_interest || "General Inquiry"}</p>
                              </div>
                    ...
                    <div className="bg-card border border-border/60 rounded-xl p-6">
                      <div className="flex justify-between items-center mb-4">
                        <h3 className="font-display text-lg uppercase text-foreground">Recent Subscriptions</h3>
                        <button onClick={() => setActiveTab("subscribers")} className="text-xs text-azure hover:underline">View all</button>
                      </div>
                      {subscribers.length === 0 ? (
                        <p className="text-sm text-muted-foreground/60 py-4 font-mono">No subscribers yet.</p>
                      ) : (
                        <div className="divide-y divide-border/40">
              ...
              {activeTab === "appointments" && (
                <div className="space-y-6">
                  <div>
                    <h2 className="font-display text-3xl uppercase text-foreground">Demo Appointments</h2>
                    <p className="text-muted-foreground text-sm mt-1">Incoming demo session requests via the Book Demo form.</p>
                  </div>
                    ...
                    if (filtered.length === 0) {
                      return (
                        <div className="bg-card border border-border/60 rounded-xl p-12 text-center">
                          ...
                          <p className="text-muted-foreground/60 font-mono text-sm">No demo bookings yet.</p>
                        </div>
                      );
                    }
                    return (
                          ...
                          <table className="w-full text-left border-collapse text-xs md:text-sm">
                            ...
                            <tbody className="divide-y divide-border/40">
                              {filtered.map(appt => {
                                const parsed = parseDemoMessage(appt.message);
                                return (
                                  <tr key={appt.id} className="hover:bg-muted/10 text-foreground/85">
                                    ...
                                    <td className="p-4 font-semibold text-azure">{appt.course_interest || "General Inquiry"}</td>
                                    <td className="p-4 font-semibold text-rose-400 font-mono">{parsed.day || "—"}</td>
                                    <td className="p-4 font-semibold text-rose-400 font-mono">{parsed.time || "—"}</td>
                ...
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      ...
                      <p className="text-muted-foreground text-sm mt-1">Add details, manage syllabus, and customize certifications.</p>
                    </div>
                    <button
                      onClick={() => {
                        setEditingCourse({
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {courses.map(course => (
                      <div key={course.id} className="bg-card border border-border/60 rounded-xl overflow-hidden shadow-md hover:border-border/80 transition-all flex flex-col">
                        {course.hero_image ? (
                          <div className="h-40 w-full overflow-hidden bg-muted/70 relative">
                            ...
                            {course.video_url && (
                              <div className="absolute top-3 right-3 bg-red-600/90 text-foreground p-1.5 rounded-full shadow-md" title="Has Video Embed">
                        ...
                        <div className="p-5 flex-1 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start">
                              ...
                              <span className="text-muted-foreground/60 text-xs font-mono">Order: {course.display_order}</span>
                            </div>
                            <h3 className="font-display text-xl uppercase mt-3 text-foreground leading-tight">{course.name}</h3>
                            <p className="text-muted-foreground text-xs mt-2 line-clamp-2 leading-relaxed">{course.tagline || course.summary}</p>
                            ...
                            {course.video_url && (() => {
                              ...
                              return (
                                <div className="mt-4 p-2 bg-background border border-border/60 rounded flex items-center gap-2 text-xs text-foreground/85 font-mono">
                                  ...
                                  <a href={course.video_url} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground shrink-0">
              ...
              {activeTab === "leads" && (
                <div className="space-y-6">
                  <div>
                    <h2 className="font-display text-3xl uppercase text-foreground">Student Leads</h2>
                    <p className="text-muted-foreground text-sm mt-1">Track admissions trial requests and contact inquiries.</p>
                  </div>
                          ...
                          {leads
                            ...
                            .map(lead => (
                              <tr key={lead.id} className="hover:bg-muted/10 text-foreground/85">
                                ...
                                <td className="p-4 font-mono text-muted-foreground/60">{lead.source || "Website"}</td>
                                <td className="p-4 font-mono text-muted-foreground/60">{new Date(lead.created_at).toLocaleDateString()}</td>
                                <td className="p-4 text-right">
                                  <div className="flex items-center justify-end gap-2">
                                    <button
              ...
              {activeTab === "subscribers" && (
                <div className="space-y-6">
                  <div>
                    <h2 className="font-display text-3xl uppercase text-foreground">Newsletter Subscribers</h2>
                    <p className="text-muted-foreground text-sm mt-1">List of readers signed up to receive monthly recitals and workshop alerts.</p>
                  </div>
                          ...
                          {subscribers
                            ...
                            .map(sub => (
                              <tr key={sub.id} className="hover:bg-muted/10 text-foreground/85">
                                ...
                                <td className="p-4 font-mono text-muted-foreground/60">{new Date(sub.created_at).toLocaleDateString()}</td>
                              </tr>
                            ))}
                        </tbody>
                      </table>
              ...
              {activeTab === "lessons" && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h2 className="font-display text-3xl uppercase text-foreground">Course Videos Console</h2>
                      <p className="text-muted-foreground text-sm mt-1">Upload, update, and manage recorded lessons, tutorials, and class materials.</p>
                    </div>
                    <button
                      onClick={() => {
                        setEditingLesson({
                        ...
                        <tbody className="divide-y divide-border/40">
                          {lessons.map(lesson => (
                            <tr key={lesson.id} className="hover:bg-muted/10 text-foreground/85">
                              ...
                              <td className="p-4 text-muted-foreground max-w-xs truncate">{lesson.description || "-"}</td>
                              ...
                              <td className="p-4 font-mono text-muted-foreground">{lesson.display_order}</td>
                              <td className="p-4 text-right">
              ...
              {activeTab === "fees" && (
                <div className="space-y-6">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                      <h2 className="font-display text-3xl uppercase text-foreground">Tuition Packages Console</h2>
                      <p className="text-muted-foreground text-sm mt-1">Create and update course packages, costs, features list, and page display order.</p>
                    </div>
                    <button
                      onClick={() => {
                        setEditingFee({
                        ...
                        <tbody className="divide-y divide-border/40">
                          {fees.map(fee => (
                            <tr key={fee.id} className="hover:bg-muted/10 text-foreground/85">
                              ...
                              <td className="p-4 font-mono text-muted-foreground">Rs. {fee.raw_fees.toLocaleString()}</td>
                              <td className="p-4 text-muted-foreground">{fee.duration}</td>
                              <td className="p-4 text-muted-foreground">{fee.mode}</td>
                              <td className="p-4">
                                <span className={`px-2 py-0.5 rounded text-[10px] font-mono uppercase font-bold ${fee.popular ? "bg-amber-500/10 text-amber-500 border border-amber-500/20" : "bg-muted text-muted-foreground border border-border/80/50"}`}>
                                  {fee.popular ? "Yes" : "No"}
                                </span>
                              </td>
                              <td className="p-4 font-mono text-muted-foreground">{fee.display_order}</td>
                              <td className="p-4 text-right">
                                <div className="inline-flex gap-2">
                                  <button
                                    onClick={() => {
            ...
            <form onSubmit={saveCourse} className="flex-1 overflow-y-auto p-6 space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Course Name *</label>
                  <input
                    type="text"
                    required
                ...
                {/* Slug */}
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">URL Slug *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="grid gap-2 md:col-span-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Tagline / Short description</label>
                  <input
                    type="text"
                ...
                {/* Summary */}
                <div className="grid gap-2 md:col-span-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Overview / Full Summary</label>
                  <textarea
                    rows={4}
                ...
                {/* Certification */}
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Certification</label>
                  <input
                    type="text"
                ...
                {/* Hero Image */}
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Hero Image URL</label>
                  <input
                    type="text"
                ...
                <div className="grid gap-2 md:col-span-2 border border-border/60/80 bg-background/25 p-4 rounded-xl">
                  ...
                  <div className="grid md:grid-cols-2 gap-4 mt-2">
                    ...
                    <div className="space-y-2">
                      <span className="font-mono text-[9px] text-muted-foreground block">OPTION A: PASTE VIDEO URL</span>
                      <input
                        type="text"
                      ...
                      />
                      <p className="text-[9px] text-muted-foreground/60 font-mono">YouTube, Vimeo, or direct video file link.</p>
                    </div>

                    {/* Option B: Upload File */}
                    <div className="space-y-2">
                      <span className="font-mono text-[9px] text-muted-foreground block">OPTION B: UPLOAD VIDEO FILE (MAX 100MB)</span>
                      <div className="relative border border-dashed border-border/60 rounded-lg p-3 flex flex-col items-center justify-center bg-background/50 hover:bg-background hover:border-azure/40 transition-all duration-300 min-h-[75px]">
                        {uploadingCourseVideo ? (
                          ...
                          <>
                            ...
                            <span className="text-[11px] font-semibold text-foreground/85">Select MP4/MOV Video</span>
                            <span className="text-[9px] text-muted-foreground/60 font-mono mt-0.5">Drag & drop or click</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  {editingCourse.video_url && (
                    <div className="mt-2 text-xs bg-muted/70/50 p-2.5 rounded border border-border/60 flex items-center justify-between">
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="truncate font-mono text-foreground/85 text-[11px]">{editingCourse.video_url}</span>
                      </div>
                      <button
                        type="button"
                ...
                {/* Display Order */}
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Display Order</label>
                  <input
                    type="number"
                ...
                {/* Levels */}
                <div className="grid gap-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Levels (comma separated)</label>
                  <input
                    type="text"
                ...
                {/* Outcomes */}
                <div className="grid gap-2 md:col-span-2">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Outcomes / Student Achievements (One per line)</label>
                  <textarea
                    rows={3}
                ...
                <div className="grid gap-4 md:col-span-2 border-t border-border/60 pt-6">
                  <div className="flex justify-between items-center">
                    <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Syllabus Terms/Chapters</label>
                    <button
                      type="button"
                      onClick={() => {
                  ...
                  {((editingCourse.curriculum as any[]) || []).length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {((editingCourse.curriculum as any[]) || []).map((term, tIdx) => (
                        <div key={tIdx} className="bg-background border border-border/60 p-4 rounded-xl space-y-3 relative">
                          <button
                            type="button"
                            onClick={() => {
                              const cur = [...(editingCourse.curriculum as any[] || [])];
                          ...
                          <div className="grid gap-1">
                            <span className="font-mono text-[9px] text-muted-foreground/60">Chapter Title</span>
                            <input
                              type="text"
                              value={term.term}
                              onChange={e => {
                          ...
                          <div className="space-y-2">
                            <span className="font-mono text-[9px] text-muted-foreground/60 block">Topics / Lessons</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-card border border-border/60 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden my-8 flex flex-col">
            ...
            <form onSubmit={saveLesson} className="p-6 space-y-4">
              {/* Title */}
              <div className="grid gap-1">
                <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Lesson Title *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid gap-2 border border-border/60/80 bg-background/20 p-4 rounded-xl">
                ...
                {editingLesson.video_url && (
                  <div className="mt-2 text-xs bg-muted/70/50 p-2.5 rounded border border-border/60 flex items-center justify-between">
                    <div className="flex items-center gap-2 truncate">
                      <span className="font-mono text-[9px] text-azure font-bold border border-azure/20 bg-azure/5 px-1.5 py-0.5 rounded">Active URL</span>
                      <span className="truncate font-mono text-foreground/85 text-[11px]">{editingLesson.video_url}</span>
                    </div>
                    <button
                      type="button"
              ...
              {/* Resource Link URL */}
              <div className="grid gap-1">
                <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Resource PDF / Sheet Music Link</label>
                <input
                  type="text"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-card border border-border/60 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden my-8 flex flex-col">
            ...
            <form onSubmit={saveFee} className="p-6 space-y-4 max-h-[75vh] overflow-y-auto">
              {/* Title */}
              <div className="grid gap-1">
                <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Package Title *</label>
                <input
                  type="text"
                  required
              ...
              {/* Tagline */}
              <div className="grid gap-1">
                <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Tagline</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Total Fee Label */}
                <div className="grid gap-1">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Total Fees (Display) *</label>
                  <input
                    type="text"
                    required
                ...
                {/* Raw Fees Number */}
                <div className="grid gap-1">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Raw Fees (Number) *</label>
                  <input
                    type="number"
                    required
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                {/* Mode */}
                <div className="grid gap-1">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Study Mode *</label>
                  <input
                    type="text"
                    required
              ...
              {/* Features List */}
              <div className="grid gap-1">
                <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Package Features (One per line) *</label>
                <textarea
                  rows={4}
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                {/* Popular Badge */}
                <div className="grid gap-1">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Popular / Highlighted?</label>
                  <select
                ...
                {/* Badge text */}
                <div className="grid gap-1">
                  <label className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Badge Text</label>
                  <input
                    type="text"
      ...
      {leadDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-card border border-border/60 rounded-xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col">
            <div className="bg-muted/60 px-6 py-4 border-b border-border/60 flex justify-between items-center">
              <h3 className="font-display text-lg uppercase text-foreground">Lead Message Detail</h3>
              <button onClick={() => setLeadDetail(null)} className="text-muted-foreground hover:text-foreground font-mono text-lg">✕</button>
            </div>
            <div className="p-6 space-y-4 text-sm leading-relaxed">
              <div className="grid grid-cols-2 gap-4 border-b border-border/60 pb-4">
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Name</span>
                  <span className="font-semibold text-foreground">{leadDetail.name}</span>
                </div>
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Course Interest</span>
                  <span className="font-semibold text-azure">{leadDetail.course_interest || "General Inquiry"}</span>
                </div>
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Email</span>
                  <span className="text-foreground/85 font-mono">{leadDetail.email}</span>
                </div>
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Phone</span>
                  <span className="text-foreground/85 font-mono">{leadDetail.phone || "—"}</span>
                </div>
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Submitted Date</span>
                  <span className="text-muted-foreground font-mono">{new Date(leadDetail.created_at).toLocaleString()}</span>
                </div>
                <div>
                  <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block">Source</span>
                  <span className="text-muted-foreground font-mono">{leadDetail.source || "Website"}</span>
                </div>
              </div>
              <div>
                <span className="font-mono text-[9px] text-muted-foreground/60 uppercase block mb-1">Message Content</span>
                <div className="bg-background border border-slate-850 p-4 rounded-lg text-foreground/85 font-sans whitespace-pre-wrap">
                  {leadDetail.message || <span className="italic text-muted-foreground/60">No message provided.</span>}
                </div>
              </div>
            </div>
